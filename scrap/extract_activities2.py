...
def update_csv_with_extracted_info(input_csv, output_csv):
    ...
    print("Columns available in CSV:", data.columns)
    
    # Update the column name based on actual column name in your CSV
    data['Google Maps Link'] = ''
    data['Hours'] = ''

    for index, row in data.iterrows():
        # Update 'URL' to the actual column name
        url = row['URL']  # Adjust based on the actual column name
